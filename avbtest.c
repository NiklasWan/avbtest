...
int parseargs(int argc, char *argv[], struct avbtestcfg *cfg)
{
    ...
    if (cfg->devname[0] == 0)
    {
        if ((cfg->mode == AVB_TEST_MODE_AVB_PLAYBACK) || (cfg->mode == AVB_TEST_MODE_AVB_DEMO_TX) || (cfg->mode == AVB_TEST_MODE_AVB_RECORD))
            ...
            memcpy(&cfg->devname[0], "hw:CARD=H58040000encode,0", strlen("hw:CARD=H58040000encode,0") + 1);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    else if (cfg.mode == AVB_TEST_MODE_AVB_DEMO_DEV_X)
    {
        ...
        cfg.sync = 1;
        cfg.timestamp = 0;
        memcpy(&cfg.devname, "hw:CARD=Black,0", strlen("hw:CARD=Black,0") + 1);
        initAndStartThread(T0, 50, &cfg, startPlayback, 0);
        cfg.sync = 0;
        cfg.timestamp = 1;
    ...
    else if (cfg.mode == AVB_TEST_MODE_AVB_DEMO_DEV_Y)
    {
        ...
        cfg.timestamp = 0;
        memcpy(&cfg.filename, "r042.wav", strlen("r042.wav") + 1);
        memcpy(&cfg.devname, "hw:CARD=H58040000encode,0", strlen("hw:CARD=H58040000encode,0") + 1);
...
void *startPlayback(void *argument)
{
    ...
    else
    {
        ...
        hdr.audioformat = 1;
        hdr.bps = 16;
        hdr.numchannels = arg->cfg.numchannels;
        hdr.blockalign = (hdr.numchannels * (hdr.bps / 8));
        hdr.samplerate = arg->cfg.samplerate;
        hdr.byterate = ((hdr.numchannels * (hdr.bps / 8)) * hdr.samplerate);
    }
    ...
    while (((numFrames < arg->cfg.maxframes) || (arg->cfg.maxframes == -1)) && (ferr == 0) && (playbackDone == 0))
    {
        ...
        {
            while ((audbufsize < (2 * readBytes)) && (playbackDone == 0))
                usleep(1000);
            memcpy(&buf[arg->id][0], &audbuf[currTxIdx], readBytes);
            currTxIdx += readBytes;
            currTxIdx %= AVB_TEST_AUD_BUFFER_SIZE;
...
void *startRecord(void *argument)
{
    ...
    while (((numFrames < arg->cfg.maxframes) || (arg->cfg.maxframes == -1)) && (playbackDone == 0))
    {
        ...
        else
        {
            ...
            audbufsize += readBytes;
            if (audbufsize > AVB_TEST_AUD_BUFFER_SIZE)
                assert(0);
            currRxIdx += readBytes;
            currRxIdx %= AVB_TEST_AUD_BUFFER_SIZE;
        }
    ...
    hdr.fmtsize = 16;
    hdr.datasize = ((hdr.numchannels * (hdr.bps / 8)) * numFrames);
